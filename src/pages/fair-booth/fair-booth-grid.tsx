...
export function FairBoothGrid({
  ...
  const renderFn = (day: FairDay): Map<number, CellRender> => {
    ...
    for (const slot of day.slots) {
      ...
      // Each signup is slot-specific — a person working both slots has two
      // separate signup rows (created together by the "Add spanning both" button).
  ...
  return (
    <div className="rounded-md overflow-hidden border-2 border-black" style={{transform: 'translateZ(0)'}}>
      <table className="w-full text-xs table-fixed" style={{borderCollapse: 'separate', borderSpacing: 0}}>
        ...
        <tbody>
          {HOUR_ROWS.map((h, rowIdx) => {
            const label = `${formatTimeShort(h * 60)} - ${formatTimeShort((h + 1) * 60)} PM`
            const axisBorderTop = rowIdx === 0 ? '' : 'border-t border-t-dashed border-t-gray-600'
            return (
              <tr key={h}>
                <td
