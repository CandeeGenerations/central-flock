...
import { getJob, cancelJob, createJob, type SendJob } from '../services/message-queue.js';
...
messagesRouter.post('/send', async (req, res) => {
  try {
    ...
    const recipients = db
      .select()
      .from(schema.people)
      .where(sql`${schema.people.id} IN (${sql.join(recipientIds.map(id => sql`${id}`), sql`, `)})`)
      .all();

    const excludeSet = new Set(excludeIds);
    const activeRecipients = recipients.filter(r => !excludeSet.has(r.id));
    const skippedRecipients = recipients.filter(r => excludeSet.has(r.id));
    ...
    for (const person of activeRecipients) {
      db.insert(schema.messageRecipients).values({
    ...
    // Start async sending
    const job = createJob(message.id, serviceType, batchSize, batchDelayMs);
    processSendJob(job);
...
messagesRouter.get('/', async (_req, res) => {
  try {
    ...
    const result = messagesList.map(msg => {
      let groupName = null;
      if (msg.groupId) {
        const group = db.select({ name: schema.groups.name }).from(schema.groups).where(eq(schema.groups.id, msg.groupId)).get();
...
messagesRouter.get('/:id', async (req, res) => {
  try {
    const message = db.select().from(schema.messages).where(eq(schema.messages.id, Number(req.params.id))).get();
    if (!message) {
    ...
    let groupName = null;
    if (message.groupId) {
      const group = db.select({ name: schema.groups.name }).from(schema.groups).where(eq(schema.groups.id, message.groupId)).get();
      groupName = group?.name || null;
    }
...
messagesRouter.post('/:id/cancel', async (req, res) => {
  try {
    ...
    db.update(schema.messageRecipients)
      .set({ status: 'skipped' })
      .where(sql`${schema.messageRecipients.messageId} = ${messageId} AND ${schema.messageRecipients.status} = 'pending'`)
      .run();
...
async function processSendJob(job: SendJob) {
  const pendingRecipients = db
    ...
    .from(schema.messageRecipients)
    .innerJoin(schema.people, eq(schema.messageRecipients.personId, schema.people.id))
    .where(sql`${schema.messageRecipients.messageId} = ${job.messageId} AND ${schema.messageRecipients.status} = 'pending'`)
    .all();
  ...
  for (let i = 0; i < pendingRecipients.length; i++) {
    ...
    try {
      await sendMessage(recipient.phoneNumber, recipient.renderedContent || '', job.serviceType);
